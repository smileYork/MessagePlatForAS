package model;

import android.net.Uri;
import android.provider.BaseColumns;

public interface SMS extends BaseColumns {

    public static final Uri CONTENT_URI = Uri.parse("content://sms");

    public static final String TYPE = "type";// 短信类型，1代表接收，2代表发送

    public static final String THREAD_ID = "thread_id";// 序号，同一发信人的id相同

    public static final String ADDRESS = "address";// 发件人手机号码

    public static final String PERSON_ID = "person";// 联系人列表里的序号，陌生人为null

    public static final String DATE = "date";// 发件日期

    public static final String READ = "read";// 是否阅读 0未读， 1已读

    public static final String BODY = "body";// 短信内容

    public static final String PROTOCOL = "protocol";// 协议，分为： 0 SMS_RPOTO, 1
						     // MMS_PROTO

    public static final int MESSAGE_TYPE_ALL = 0;

    public static final int MESSAGE_TYPE_INBOX = 1;

    public static final int MESSAGE_TYPE_SENT = 2;

    public static final int MESSAGE_TYPE_DRAFT = 3;

    public static final int MESSAGE_TYPE_OUTBOX = 4;

    public static final int MESSAGE_TYPE_FAILED = 5; // for failed outgoing
						     // messages

    public static final int MESSAGE_TYPE_QUEUED = 6; // for messages to send
						     // later

    public static final int PROTOCOL_SMS = 0;// SMS_PROTO

    public static final int PROTOCOL_MMS = 1;// MMS_PROTO
}
